...
def title1(): # Update the title IF option 1 was picked.
    ...
    while True:
        time_elapsed = strftime('%H:%M:%S', gmtime(time() - start))
        system(f'title TiktokBot ^| Views Sent: {beautify(Views)} ^| Elapsed Time: {time_elapsed}')
...
def title2(): # Update the title IF option 2 was picked.
    ...
    while True:
        time_elapsed = strftime('%H:%M:%S', gmtime(time() - start))
        system(f'title TiktokBot ^| Hearts Sent: {beautify(Hearts)} ^| Elapsed Time: {time_elapsed}')
...
def title3(): # Update the title IF option 3 was picked.
    ...
    while True:
        time_elapsed = strftime('%H:%M:%S', gmtime(time() - start))
        system(f'title TiktokBot ^| Followers Sent: {beautify(Followers)} ^| Elapsed Time: {time_elapsed}')
...
def title4(): # Update the title IF option 1 was picked.
    ...
    while True:
        time_elapsed = strftime('%H:%M:%S', gmtime(time() - start))
        system(f'title TiktokBot ^| Shares Sent: {beautify(Shares)} ^| Elapsed Time: {time_elapsed}')
...
if auto == 1:
    ...
    print("[+] This program was update improved by @AeX03. [github.com/AeX03]")
